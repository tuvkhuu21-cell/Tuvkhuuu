...
function DriverDashboardSection({ onBack }) {
  ...
  // Fetch all active orders (we'll filter for unassigned in the UI)
  ...
  return allActiveOrders.filter(order => 
    ...
    order.workflowStatus === 'Unassigned'
  )
}, [allActiveOrders])
  ...
  async function loadDriverData() {
    ...
    const [profileData, pendingData, activeData, completedData, allActiveData] = await Promise.all([
    ...
    setAllActiveOrders(allActiveData)
  ...
  return (
    ...
    <section className="mx-auto w-full max-w-7xl overflow-hidden rounded-xl border border-white/10 bg-[#05070d] shadow-[0_24px_80px_rgba(0,0,0,0.55)]">
      ...
      <div className="space-y-4 px-4 py-5 md:px-6 md:py-6">
        ...
        {loading || auth.loading ? <p className="text-sm text-white/65">Loading driver data...</p> : null}
        ...
        <article className="rounded-lg border border-white/10 bg-white/2 p-4 md:p-5">
          ...
          <div className="mb-3 flex items-center justify-between">
            ...
            <h2 className="text-2xl font-semibold text-white/92">Unassigned Orders</h2>

            <span className="rounded bg-white/10 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-white/70">{unassignedOrders.length}</span>

          </div>

          {unassignedOrders.length === 0 ? <p className="text-sm text-white/60">No unassigned orders available.</p> : null}

          <div className="space-y-2">

            {unassignedOrders.map((order) => (
              <article key={order.id} className="rounded-md border border-white/10 bg-black/20 px-3 py-2">
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-white/60">{order.pickupAddress} → {order.dropoffAddress}</p>

                    <p className="text-xs text-white/45 mt-1">Status: <span className="text-yellow-400">{order.workflowStatus || order.status}</span></p>

                    <p className="text-xs text-white/45 mt-1">Driver: <span className="text-green-400 font-semibold">Available for Acceptance</span></p>
                  ...
                  <div className="flex gap-1">
                    ...
                    <button
                      ...
                      onClick={() => runAction(`accept-${order.id}`, () => api.acceptAvailableOrder(order.id))}

                      className="rounded bg-[#8ea8ff] px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#071327] hover:bg-[#a2b8ff]"
                    ...
                    <button
                      ...
                      disabled={busyKey === `all-detail-${order.id}`}

                      onClick={() => runAction(`all-detail-${order.id}`, async () => {
                        try {
                          setSelectedOrderDetail(await api.getDriverOrder(order.id))
                        } catch (error) {
                          // If order is not assigned to this driver, show basic info
                          setSelectedOrderDetail(order)
                        }
                      })}

                      className="rounded border border-white/16 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-white/80 hover:bg-white/6"
